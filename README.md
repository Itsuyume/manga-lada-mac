...
- BallonsTranslator 엔진이 있으면 텍스트 검출, OCR, 번역, 지우기, 식자를 한 번에 실행
...
- Ballons 실행 결과: `Application Support/Manga Lada/BallonsRuns`
...
첫 번역에서는 text detector와 manga OCR 모델을 내려받기 때문에 시간이 걸릴 수 있습니다. BallonsTranslator는 GPL-3.0 계열 프로젝트라서, 현재 앱은 해당 소스를 번들에 포함하지 않고 사용자 로컬의 외부 엔진으로 호출합니다.
